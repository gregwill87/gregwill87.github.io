$(document).ready(function(){
	...
	$('.generate-btn').on('click', function(){
		if($('#ccUsername').val() != '') {
			if($('#ccGems select').val() != ''&& $('#ccGold select').val() != '') {
				...
				sweetAlert("Fehler", "Veuillez entrez une valeur pour les ressources gold et gemmes SVP.", "error");
			}
		} else {
			sweetAlert("Fehler", "Geben Sie Ihren Clash Royale Nutzernamen ein.", "error");
		}
	});
	...
	function confirmDialogOpen(gems, gold, callback){
		bootbox.dialog({
			message: "<p>Bist Du sicher, dass Du die Anzahl an Ressourcen Deinem Konto gutschreiben lassen willst?</p><p><img src='img/gems-icon.png' height='20px'/> " + gems + ".</p><p><img src='img/gold-icon.png' height='20px'/> " + gold + ".</p>",
